...
- verifyBackfill now fails closed on polymorphic rows whose discriminator is not one of the declared mappings (or is NULL): such rows are never backfilled, and the report names the unmapped discriminator values instead of silently passing.
